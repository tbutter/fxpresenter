...
public class Style extends PElement, StyleProperties {
	postinit {
            nsURI = nsSTYLE;
	...
	public function getStyleName() : String {
		return getAttributeNS(nsSTYLE, "name").value;
	}

	public function isAutomaticStyle() : Boolean {
		return parent.localName.equals("automatic-styles");
	}

	override public function addToStyleMap(map : StyleMap, level : Integer) {
            for(n in children where n instanceof StyleProperties) {
                (n as StyleProperties).addToStyleMap(map, level);
            }
	}

	public function getFamily() : String {
		return getAttributeNS(nsSTYLE, "family").value;
	}
}
