...
public class ListLevelStyle  extends PElement, StyleProperties {

	public function getLevel() : Integer {
		return Integer.parseInt(getAttributeNS(nsTEXT, "level").value);
	}

	override public function addToStyleMap(map : StyleMap, level : Integer) {
            for(node in children) {
                if (node instanceof StyleProperties) {
